...
describe("Integration - MongoDbAlbumRepository", () => {
  ...
  it("Should get album by id", async () => {
    const result = await mongoDbAlbumRepository.getAlbumById(
      album.props.albumId
    );
  ...
  it("Should get album by userId", async () => {
    const result = await mongoDbAlbumRepository.getAlbumByUserId(
      album.props.userId
    );
    await expect(result).toEqual(album);
  });
  ...
  it("Should get album by title", async () => {
    const result = await mongoDbAlbumRepository.getAlbumByTitle(
  ...
  it("should return true if album presentation exists", async () => {
    await mongoDbAlbumRepository.create(album2);
    const test = await mongoDbAlbumRepository.exist("WMCA", "village people");
    console.log();
    await expect(test).toBeTruthy();
  });

  it("should return false if album presentation doesnt exist", async () => {
    const result = await mongoDbAlbumRepository.exist("WMCA", "village people");
    await expect(result).toBeFalsy();
  });
  it("Should delete album ", async () => {
    const result = await mongoDbAlbumRepository.deleteAlbum(
      album.props.albumId
    );
    await expect(result).toBeFalsy();
  });
