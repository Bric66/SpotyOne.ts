...
export class InMemoryTrackRepository implements TrackRepository {
  ...
  async getTracks(): Promise<string[]> {
    ...
    const result = tracks.map(
      (track) =>
        `title: ${track.props.trackTitle}, artist: ${track.props.artist}`
    );
    return result;
  }
  ...
  async exist(trackTitle: string, artist: string): Promise<Boolean> {
    const values = Array.from(this.db.values());
    const isTrackExist = values.find(
      (elm) =>
        elm.props.trackTitle === trackTitle && elm.props.artist === artist
    );
    if (isTrackExist) {
      return true;
    }
  ...
  async getTracksByDescendingDate(): Promise<Object[]> {
    const tracks = Array.from(this.db.values());
    tracks.sort((a, b) => +a.props.created - +b.props.created);
